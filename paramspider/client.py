...
def write_failed_to_file(url):
    ...
    with open(failed_urls,"w") as f:
        clean_url = parse_qs(urlparse(url).query).get('url', [None])[0]
        f.write(clean_url)
...
def fetch_url_content(url,proxy,timeout=3):
    """
    ...
    logging.error(f"Failed to fetch URLs {url} after {MAX_RETRIES} retries. Moving on to next domain")
